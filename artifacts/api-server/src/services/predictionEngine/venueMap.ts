...
const TOURNAMENT_VENUES: Array<{ match: RegExp; name: string; latitude: number; longitude: number; timezone: string }> = [
  ...
  { match: /\bus open\b/i, name: "New York, USA", latitude: 40.7498, longitude: -73.8459, timezone: "America/New_York" },
  ...
  { match: /^rome\b|italian open/i, name: "Rome, Italy", latitude: 41.9284, longitude: 12.4805, timezone: "Europe/Rome" },
  { match: /canadian open|\bmontreal\b|\btoronto\b/i, name: "Toronto/Montreal, Canada", latitude: 43.6532, longitude: -79.3832, timezone: "America/Toronto" },
  { match: /\bcincinnati\b/i, name: "Cincinnati, USA", latitude: 39.2331, longitude: -84.5610, timezone: "America/New_York" },
  { match: /\bhalle\b/i, name: "Halle, Germany", latitude: 51.4826, longitude: 8.2895, timezone: "Europe/Berlin" },
  { match: /queen'?s club|\bqueens\b/i, name: "London, UK", latitude: 51.4817, longitude: -0.2154, timezone: "Europe/London" },
  { match: /\bbarcelona\b/i, name: "Barcelona, Spain", latitude: 41.3851, longitude: 2.1734, timezone: "Europe/Madrid" },
  { match: /\bhamburg\b/i, name: "Hamburg, Germany", latitude: 53.5511, longitude: 9.9937, timezone: "Europe/Berlin" },
  { match: /\bdubai\b/i, name: "Dubai, UAE", latitude: 25.2048, longitude: 55.2708, timezone: "Asia/Dubai" },
  { match: /\bacapulco\b/i, name: "Acapulco, Mexico", latitude: 16.8531, longitude: -99.8237, timezone: "America/Mexico_City" },
  { match: /\bdoha\b|\bqatar\b/i, name: "Doha, Qatar", latitude: 25.2854, longitude: 51.5310, timezone: "Asia/Qatar" },
];

// See surfaceMap.ts's identical guard for why this exists: short single-word entries above (e.g.
// "halle") can appear as a pure substring inside unrelated lower-tier tournament names (e.g.
// "Challenger"). No major/Masters/500 venue in the table above is ever named with these words.
...
export function inferVenue(tournamentName: string | null | undefined): Venue | null {
  if (!tournamentName || NEVER_NAMED_TABLE.test(tournamentName)) return null;
